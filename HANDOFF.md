...
Next: connect Lightning as the bounded default planner and retain Ultra as an explicit escalation path, with per-request cloud disclosure and no execution authority. Add Tavily only for a separate useful workflow. Nmap fixture parsing/scope validation remains the next independent security milestone. Keep live scans and remote operations gated on an enrolled target and explicit scope.
