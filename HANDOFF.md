...
Next: let the user test live Lightning planning and the Tavily Nigeria source panel through the private loopback URL. Then add an explicit in-app approval/review state before any future provider-backed workflow, while keeping task execution blocked. Nmap fixture parsing/scope validation remains the next independent security milestone. Keep live scans and remote operations gated on an enrolled target and explicit scope.
