...
- Live model discovery was performed with the locally configured Nebius key without exposing it. The account returned four NVIDIA candidates. Selected primary: `nvidia/Nemotron-3-Ultra-550b-a55b` for strongest complex reasoning; fallback: `nvidia/Nemotron-3.5-Lightning` for lower-cost/faster calls. Both expose reasoning/tools metadata; SUDO X still grants no tool authority.
...
Next: benchmark Ultra versus Lightning on a small frozen SUDO X task set, then connect only a bounded provider-backed planning path with explicit cloud disclosure and no execution authority. Add Tavily only for a separate useful workflow. Nmap fixture parsing/scope validation remains the next independent security milestone. Keep live scans and remote operations gated on an enrolled target and explicit scope.
