...
Next: let the user test the live Lightning planner preview through the private loopback URL. Then add an explicit in-app approval/review state before any future provider-backed workflow, while keeping task execution blocked. Add Tavily only for a separate useful workflow. Nmap fixture parsing/scope validation remains the next independent security milestone. Keep live scans and remote operations gated on an enrolled target and explicit scope.
