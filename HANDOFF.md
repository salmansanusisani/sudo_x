...
Next: verify a user-approved synthetic probe with an account-configured NVIDIA model and spending limit, then add the UI disclosure/approval flow before connecting any general planner. Nmap fixture parsing/scope validation remains the next independent security milestone. Keep live scans and remote operations gated on an enrolled target and explicit scope.
...
The synthetic transport and CLI disclosure preview now exist in `src/sudo_x/probe.py` and have offline tests. Next: validate a separately approved live synthetic call with the user's exact account model and account-side spending limit; then implement the GUI disclosure/approval flow. Keep general requests blocked until structured tool calling, cloud disclosure, budgets, and deterministic policy checks exist. Nmap fixture parsing and scope validation can proceed independently before any live authorized scan. Remote diagnostics precede remote mutation.
