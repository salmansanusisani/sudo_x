...
class Store:
    ...
    def __init__(self, directory: Path):
        ...
        try:
            ...
            # Prevent a second launcher from reconciling this process's live tasks.
            try:
    ...
    def create(self, task_input: TaskInput) -> Task:
        ...
        with self.connection:
            ...
            self.connection.execute(
                ...
                (task_id, task_input.prompt, task_input.kind, timestamp, timestamp,
                 "Queued for a bounded local task.", scene),
            )
            self.connection.execute(
    ...
    def advance(
        ...
        status: Status = "running", result: dict | None = None,
    ) -> bool:
        with self.connection:
            ...
            self.connection.execute(
                ...
                (status, timestamp, message, json.dumps(result) if result is not None else None,
                 task_id),
            )
            self.connection.execute(
