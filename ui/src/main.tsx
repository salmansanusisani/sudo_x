...
import { api, bootstrapToken, isActive, type BackendStatus, type CapabilityList, type ChatResponse, type NmapFixture, type PlannerPreview, type ResearchResult, type ReviewReceipt, type Snapshot, type Task } from './api'
...
function App() {
  ...
  const [nmapFixture, setNmapFixture] = useState<NmapFixture | null>(null)
  ...
  async function loadNmapFixture() {
    if (!backend || busy) return
    const xml = '<nmaprun><host><status state="up"/><address addr="192.0.2.10"/><ports><port protocol="tcp" portid="8080"><state state="open"/><service name="http"/></port></ports></host></nmaprun>'
    const fallback: NmapFixture = {
    ...
    setBusy(true); setError(''); setNmapFixture(fallback)
    try { setNmapFixture(await api<NmapFixture>('/security/nmap-fixture', token, { xml, scope: ['192.0.2.0/24'] })) }
    catch (e) { setError(e instanceof Error ? e.message : 'Fixture parser connection failed; showing the fixed offline fixture.') }
    finally { setBusy(false) }
  }
  ...
  async function reviewContent(kind: 'planner' | 'research', content: object) {
    try {
      setReview(await api<ReviewReceipt>('/reviews', token, { kind, content }))
    } catch (e) { setError(e instanceof Error ? e.message : 'Review receipt could not be recorded.') }
  }
  ...
  function toggleVoiceInput() {
    ...
    if (!SpeechRecognition) {
      setError('Voice input is unavailable in this browser. Use Chromium with microphone permission, or continue in text mode. Your microphone was not accessed.')
      return
    }
    ...
    recognition.onerror = () => {
      ...
      setError('Voice input could not start or was denied. Check browser microphone permission, then try again or use text mode.')
    }
    ...
    <div className="app-body">
      ...
      <main>
        ...
        {section === 'control' ? <>
         ...
         <SecurityLab data={nmapFixture} disabled={!backend || busy} onLoad={() => void loadNmapFixture} visible={section === 'security'}/>
         {section === 'code' && <CodeWorkspace token={token} api={api} disabled={!backend || busy}/>}
       </main>
    </div>
