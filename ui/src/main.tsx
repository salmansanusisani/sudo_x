...
import { api, bootstrapToken, isActive, type BackendStatus, type Snapshot, type Task } from './api'
...
function App() {
  ...
  useEffect(() => {
    if (!token) return
    let cancelled = false
    Promise.all([api<BackendStatus>('/status', token), api<{ tasks: Task[] }>('/tasks', token)])
      .then(([status, data]) => { if (!cancelled) { setBackend(status); setTasks(data.tasks) } })
  ...
  return <div className={`app ${motion ? '' : 'reduced-motion'}`}>
    ...
    <div className="app-body">
      ...
      <main>
        ...
        <footer className="app-footer"><span><span className="status-dot green"/> LOCAL-FIRST <span className="footer-divider">/</span> HUMAN IN CONTROL</span><span>SUDO X <span className="footer-divider">/</span> BUILT FOR YOUR WORLD <Radio size={13}/></span></footer>
      </main>
    </div>
